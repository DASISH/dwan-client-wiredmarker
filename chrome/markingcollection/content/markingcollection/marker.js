...
/////////////////////////////////////////////////////////////////////
	init : function(){
		...
		if(alignment == undefined) nsPreferences.setIntPref("wiredmarker.hyperanchor.alignment", this.DEFAULT_ALIGMENT);
	},
...
/////////////////////////////////////////////////////////////////////
	updateMarkerWindowCount : function(doc){
		...
		for(var i=0;i<elms.length;i++){
			var elm = elms[i];
			if(elm.id.search(search_exp)<0) continue;
...
/////////////////////////////////////////////////////////////////////
	getTextNode : function(aNode){
		var rtn_txt = "";
		if(aNode.nodeName == "SELECT") return rtn_txt;
		if(aNode.style && aNode.style.display == 'none') return rtn_txt;
		if(aNode.childNodes.length){
			var elements = aNode.childNodes;
			for(var j=0;j<elements.length;j++){
				if(aNode.nodeName == "TR" && j>0) rtn_txt += "\t";
				rtn_txt += addSelectionNode(elements[j]);
			}
		}
		if(aNode.nodeName.search(/^P|H1|H2|H3|H4|H5|H6|UL|OL|DIR|MENU|PRE|DL|DIV|CENTER|NOSCRIPT|NOFRAMES|BLOCKQUOTE|FORM|ISINDEX|HR|TABLE|TR|FIELDSET|ADDRESS|MULTICOL|BR$/img)==0) rtn_txt = "\n" + rtn_txt + "\n";
...
/////////////////////////////////////////////////////////////////////
	xPathMarker : function(aDoc, aXPath, aAttributes){
		try{
			...
			if(xContext != "" && xContext != rContext && (this._isTextNode(range.startContainer) || this._isTextNode(range.endContainer))){
				...
				if(textContentP.lastIndexOf(xContext)<0 && textContentP.length<alignment){
					while(nodeWalker.previousNode()){
						...
						if(textContentP.lastIndexOf(xContext)>=0) break;
						if(textContentP.length>alignment) break;
					}
				}
				var indexP = textContentP.lastIndexOf(xContext);
				var lengthP = 0;
				if(indexP>=0) lengthP = textContentP.length;
				nodeWalker.currentNode = range.startContainer;
				...
				if(textContentN.indexOf(xContext)<0 && textContentN.length<alignment){
					while(nodeWalker.nextNode()){
						...
						if(textContentN.indexOf(xContext)>=0) break;
						if(textContentN.length>alignment) break;
					}
				}
			...
			if(doc.getElementById(aAttributes.id)) return rtnNode;
			...
			if(!createNode && sameNode) this.markerRange(range,aAttributes);
			range.collapse(true);
			if(aAttributes.id){
