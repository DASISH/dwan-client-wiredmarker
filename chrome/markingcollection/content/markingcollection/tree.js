var mcTreeHandler = {
    get TREE() {
        return document.getElementById("mcTree");
    },
    get TREE_POPUP() {
        return document.getElementById("mcToolbarRootFolderButton");
    },
    get POPUP() {
        return document.getElementById("mcPopup");
    },
    get POPUP_FOLDER() {
        return document.getElementById("mcPopupFolder");
    },
    get POPUP_OBJECT() {
        return document.getElementById("mcPopupObject");
    },
    get DataSource() {
    ...
    init: function(isContainer) {
        try {
            ...
            if (isContainer)
                document.getElementById("mcTreeRule").setAttribute("iscontainer", true);
    ...
    pageshow: function() {
        try {
            ...
            if (selectIdx >= 0) {
                ...
                if (!mcTreeHandler.TREE.view.selection.isSelected(mcTreeHandler.TREE.currentIndex))
                    mcTreeHandler.TREE.view.selection.select(mcTreeHandler.TREE.currentIndex);
        ...
        switch (aEvent.keyCode) {
            ...
            case aEvent.DOM_VK_DELETE :
                ...
                if (!(aEvent.ctrlKey || aEvent.shiftKey)) {
                    ...
                    if (type == "localfolder")
                        return;
                    if (type == window.top.bitsMarkingCollection._uncategorized.dbtype)
                        return;
    ...
    onDblClick: function(aEvent) {
        if (aEvent.originalTarget.localName != "treechildren" || aEvent.button != 0)
            return;
        if (this.TREE.view.isContainer(this.TREE.currentIndex))
            return;
        if (aEvent.button == 2) {
    ...
    existsFolderTitle: function(aRes, aTitle) {
        ...
        for (i = 0, len = contResList.length; i < len; i++) {
            if (aRes.Value == contResList[i].Value)
                continue;
            if (this.DataSource.getProperty(contResList[i], "title") == aTitle)
                return true;
        }
        return false;
    },
    create: function(pIdx) {
        ...
        var curRes = null;
        if (pIdx >= 0) {
            if (!mcTreeHandler.TREE.view.isContainer(curIdx))
                return;
            curRes = mcTreeHandler.TREE.builderView.getResourceAtIndex(curIdx);
            if (!mcTreeHandler.TREE.view.isContainerOpen(curIdx))
                mcTreeHandler.TREE.view.toggleOpenState(curIdx);
        } else {
    ...
    removeFolder: function(aEvent) {
        if (!mcController.confirmRemovingFor(this.resource))
            return false;
        if (aEvent)
            window.top.bitsScrapPartyAddonService.eventListener(aEvent, 0, this.resource);
        var aMode = this.DataSource.getProperty(this.resource, "dbtype");
        if (!this.Database.beginTransaction(aMode))
            return false;
        var curIdx = this.TREE.currentIndex;
        ...
        if (aEvent)
            window.top.bitsScrapPartyAddonService.eventListener(aEvent, 1, this.resource);
    },
    _removeFolder: function(aRes) {
        ...
        try {
            ...
            for (i = foldResList.length - 1; i >= 0; i--) {
                ...
                if (!this.DataSource.data.ArcLabelsOut(foldResList[i]).hasMoreElements() || this.Common.RDFCU.indexOf(this.DataSource.data, pRes, foldResList[i]) < 0)
                    continue;
                delObj.push({
    ...
    removeAllFolder: function(aEvent) {
        ...
        try {
            ...
            var i, j;
            for (i = foldResList.length - 1; i >= 0; i--) {
                if (foldResList[i].Value == curRes.Value)
                    continue;
        ...
        try {
            ...
            for (i = foldResList.length - 1; i >= 0; i--) {
                ...
                for (j = 0; j < contResList.length; j++) {
                    if (!this.DataSource.data.ArcLabelsOut(contResList[j]).hasMoreElements() || this.Common.RDFCU.indexOf(this.DataSource.data, foldResList[i], contResList[j]) < 0)
                        continue;
                    delObj.push({
                        pfid: pfid,
            ...
            for (var i = 0; i < delObj.length; i++) {
                ...
                if (delObj[i].source.match(match_exp))
                    bitsMarker.unmarkerWindow(delObj[i].source);
            }
        } catch (e) {
    ...
    remove: function(aEvent, pRes, pConfirm, aTransaction) {
        ...
        if (aTransaction == undefined)
            aTransaction = true;
        if (pRes == undefined && this.TREE.view.selection.count == 0)
            return false;
        if (!pConfirm)
            if (!mcController.confirmRemovingFor(this.resource))
                return false;
        ...
        if (this.TREE.view.selection.count > 1) {
            ...
            var match_exp = new RegExp("^" + bitsMarker.id_key + ".+$", "m");
            var source = this.DataSource.getProperty(curRes, "source");
            if (source.match(match_exp))
                bitsMarker.unmarkerWindow(source);
            var id = this.DataSource.getProperty(curRes, "id");
            var dbtype = this.DataSource.getProperty(curRes, "dbtype");
            ...
            if (aTransaction)
                if (!this.Database.beginTransaction(dbtype))
                    return false;
            ...
            if (aTransaction)
                this.Database.endTransaction(dbtype);
        }
    ...
    replacedURL: function(aEvent) {
        ...
        for (i = 0; i < tmpFolderFilterArr.length; i++) {
            if (!tmpFolderFilterArr[i].match(/^(\d+):(\d+):(.+)$/))
                continue;
        ...
        if (button == 0) {
            ...
            for (var i = 0; i < folderList.length; i++) {
                ...
                if (!rtn)
                    continue;
                if (folderFilterHash[fid])
                    rtn = rtn.filter(function(element, index, array) {
        ...
        if (this._replacedURL_window && !this._replacedURL_window.closed)
            this._replacedURL_window.close();
    ...
    _replacedURL_CB: function(aResult) {
        ...
        if (aResult.accept) {
            ...
            if (self.progressWindow && !self.progressWindow.closed) {
                if (aResult.objArr.length > 0) {
                    if (self.progressWindow.setStatus)
                        self.progressWindow.setStatus(mcMainService.STRING.getString("MSG_REPLACED_URL") + "... [ " + aResult.objArr.length + " ]");
                    self.progressWindow.focus();
                    setTimeout(function() {
                        mcTreeHandler._replacedURL(aResult.objArr);
                    }, 0);
                } else {
                    if (self.progressWindow && !self.progressWindow.closed)
        ...
        if (self._replacedURL_window && !self._replacedURL_window.closed)
            self._replacedURL_window.close();
        self._replacedURL_window = null;
    },
    _replacedURL: function(aObjArr) {
        if (this.progressWindow && !this.progressWindow.closed) {
            if (aObjArr.length > 0) {
                if (this.progressWindow.setStatus)
                    this.progressWindow.setStatus(mcMainService.STRING.getString("MSG_REPLACED_URL") + "... [ " + aObjArr.length + " ]");
        ...
        delete obj.dbtype;
        if (obj.fid_style != undefined)
            delete obj.fid_style;
        if (obj.fid_title != undefined)
            delete obj.fid_title;
        if (obj.folder_order != undefined)
            delete obj.folder_order;
        var rtn = this.Database.updateObject(obj, dbtype);

        setTimeout(function() {
            mcTreeHandler._replacedURL(aObjArr);
        }, 0);
    },
    ...
    getSelection: function(idx2res, rule) {
        ...
        for (rc = 0; rc < this.TREE.view.selection.getRangeCount(); rc++) {
            ...
            for (i = start.value; i <= end.value; i++) {
                if (rule == 1 && !this.TREE.view.isContainer(i))
                    continue;
                if (rule == 2 && this.TREE.view.isContainer(i))
                    continue;
    ...
    toggleFolder: function(aIdx) {
        if (!aIdx)
            aIdx = this.TREE.currentIndex;
        this.TREE.view.toggleOpenState(aIdx);
        if (this.autoCollapse)
            this.collapseFoldersBut(aIdx);
    },
    toggleAllFolders: function(forceClose) {
        ...
        for (i = 0; i < this.TREE.view.rowCount; i++) {
            if (!this.TREE.view.isContainer(i))
                continue;
            if (this.TREE.view.isContainerOpen(i)) {
                willOpen = false;
                break;
            }
        }
        ...
        if (willOpen) {
            for (i = 0; i < this.TREE.view.rowCount; i++) {
                if (this.TREE.view.isContainer(i) && !this.TREE.view.isContainerOpen(i))
...
var mcController = {
    ...
    onPopupShowing: function(aEvent) {
        ...
        if (type.indexOf("folder") >= 0) {
            ...
            for (var i = 0; i < folderList.length; i++) {
                var fid = this.DataSource.getProperty(folderList[i], "id");
                if (!this.Database.existsObject({pfid: fid}, dbtype))
                    continue;
                existsObject = true;
                break;
            }
            ...
            try {
                ...
                if (this.DataSource.isContainer(aRes)) {
                    ...
                    if (fid == "0" && dbtype == window.top.bitsMarkingCollection._uncategorized.dbtype) {
                        ...
                        if (this.Database.updateFolder({fid: fid, fid_style: style}, dbtype))
                            this.DataSource.setProperty(aRes, "style", style);
                    }
...
var mcMainService = {
    get STRING() {
        return document.getElementById("mcMainString");
    },
    get DataSource() {
        return window.top.bitsObjectMng.DataSource;
    },
        ...
        try {
            ...
            if (window.top.bitsScrapPartyAddonService && window.top.bitsScrapPartyAddonService.existsAddon()) {
                ...
                if (addon_info_arr && addon_info_arr.length > 0) {
                    ...
                    for (i = 0; i < addon_info_arr.length; i++) {
                        if (!addon_info_arr[i].side_toolbar)
                            continue;
                        if (!addon_info_arr[i].side_toolbar.attrs.xul)
                            continue;
                        try {
            ...
            try {
                if (nsPreferences.getBoolPref("wiredmarker.debug", false))
                    document.getElementById("mcToolbarMenuButton").removeAttribute("hidden");
            } catch (e) {
            }
            try {
                mcTreeViewModeService.rebuild();
    ...
    refresh: function() {
        ...
        try {
            ...
            if (mciconimage)
                mciconimage.parentNode.removeChild(mciconimage);
            mcTreeHandler.done();
            mcTreeDNDHandler.quit();
            ...
            this.Common.alert('done():' + ex);
        }
        try {
            window.top.bitsMarkingCollection._contentWindow = null;
        } catch (e) {
        }
    },
    locate: function(aRes) {
        if (aRes.Value == window.top.bitsObjectMng.DataSource.ABOUT_ROOT)
            return;
        var resList = [aRes];
        var i;
        for (i = 0; i < 32; i++) {
        ...
        for (i = 0; i < resList.length; i++) {
            ...
            if (!mcTreeHandler.TREE.view.isContainerOpen(idx))
                mcTreeHandler.TREE.view.toggleOpenState(idx);
        }
...
var mcTreeDNDHandler = {
    ...
    nsDragAndDrop_drop: function(aEvent, aDragDropObserver) {
        if (!("onDrop" in aDragDropObserver))
            return;
        if (!nsDragAndDrop.checkCanDrop(aEvent, aDragDropObserver))
            return;
    ...
    builderObserver: {
        canDrop: function(index, orient) {
                ...
                try {
                    if (mcTreeDNDHandler._dataTransfer) {
                        ...
                        if (types.length) {
                            ...
                            if (!XferData.data)
                                XferData.data = dataTransfer.mozGetDataAt(types[0], 0);
                        }
                        if (XferData.data) {
                            doraggedData = XferData.data;
                            XferDataType = XferData.flavour.contentType;
            ...
            try {
                ...
                if (dbtype == "")
                    return false;
                if (res[0] == mcTreeHandler.TREE.ref && !(editmode & 0x1000))
                    return false;
                if (mcTreeHandler.TREE.currentIndex >= 0 && XferDataType == "moz/rdfitem") {
                    ...
                    if (res[0] != mcTreeHandler.TREE.ref && (d_editmode & 0x1000))
                        return false;
                    if (res[0] != mcTreeHandler.TREE.ref && ((editmode & 0x1000) && (d_editmode & 0x1000)))
                        return false;
                } else {
                    if (editmode & 0x1000) {
                        if (XferDataType == "text/x-moz-url" || XferDataType == "application/x-moz-file") {
                            ...
                            if (!aFile || !aFile.exists() || !url)
                                return false;
                            var splitFileName = mcTreeHandler.Common.splitFileName(url);
                            if (splitFileName[1].toLowerCase() != "xml" && splitFileName[1].toLowerCase() != "zip")
                                return false;
                        } else {
                            return false;
                        }
                        ...
                        if (prevEditmode & 0x1000)
                            return false;
                    }
                }
                if (index != -1 && !mcTreeHandler.TREE.view.isContainer(index) && orient == 0)
                    return false;
...
//			try{
            if (XferDataType == "moz/rdfitem") {
                ...
                if (
                    ...
                    if (dst_fid == pfid && dst_dbtype == dbtype)
                        return;
                        ...
                        setTimeout(function() {
                            try {
                                ...
                                if (mcItemView.isChecked) {
                                    ...
                                    if (rtnFolder && rtnFolder.length)
                                        f_pfid = rtnFolder[0].pfid;
                                    rtnFolder = undefined;
                                    if (dbtype == window.top.bitsMarkingCollection._uncategorized.dbtype || mcTreeDNDHandler.modShift) {
...
//			}catch(ex){_dump2("onDrop():"+ex);}
    ...
    setDataTransfer: function(aEvent) {
        try {
            if (aEvent.dataTransfer)
                this._dataTransfer = aEvent.dataTransfer;
        } catch (e) {
        }
    },
    onMousedown: function(aEvent) {
        try {
            mcTreeDNDHandler.rangeParent = aEvent.rangeParent;
        } catch (ex) {
        }
        try {
            mcTreeDNDHandler.originalTarget = aEvent.originalTarget;
        } catch (ex) {
        }
    },
    init: function() {
    ...
    quit: function() {
        try {
            this.gBrowser.removeEventListener("mousedown", this.onMousedown, false);
        } catch (ex) {
        }
    },
    ...
    moveMultiple: function() {
        ...
        if (mcTreeHandler.validateMultipleSelection(idxList) == false)
            return;
        var i = 0;
        var curResList = [];
        var curParList = [];
    ...
    moveAfterChecking: function(curRes, curPar, tarRes, tarPar) {
        ...
        if (this.DataSource.isContainer(this.rowRes)) {
            ...
            if (rtnFolder && rtnFolder.length)
                pfid_order = rtnFolder[0].pfid_order;
        } else {
            ...
            if (rtnObject && rtnObject.length)
                pfid_order = rtnObject[0].pfid_order;
        }
        if (this.orient >= 0)
            pfid_order = parseFloat(parseFloat(pfid_order) + parseFloat(this.orient));
        if (curAbsIdx >= 0 && mcTreeHandler.TREE.view.isContainer(curAbsIdx)) {
            ...
            if (!rtn) {
                ...
                if (rtn) {
                    ...
                    var i;
                    for (i = 0; i < contResList.length; i++) {
                        if (contResList[i].Value == curRes.Value)
                            continue;
    ...
    moveFolder: function(curRes, curPar, tarPar, tarRelIdx) {
        ...
        for (i = 0; i < fldResList.length; i++) {
            ...
            if (!moveFolder || moveFolder.length == 0)
                break;
            moveFolder = moveFolder[0];
            delete moveFolder.dbtype;
            if (fid2newfid["" + moveFolder.fid])
                moveFolder.fid = fid2newfid["" + moveFolder.fid];
            ...
            if (rtn) {
                ...
                if (moveRes)
                    mcTreeHandler._removeFolder(moveRes);
                break;
            }
        }
        if (mcItemView.isChecked) {
            ...
            if (rtn) {
                for (i = 0; i < objResList.length; i++) {
                    ...
                    if (!moveObjects || moveObjects.length == 0)
                        break;
                    for (j = 0; j < moveObjects.length; j++) {
                        if (fid2newfid["" + moveObjects[j].pfid] == undefined)
                            continue;
                    ...
                    if (fid2newfid["" + moveObject.pfid])
                        moveObject.pfid = fid2newfid["" + moveObject.pfid];
    ...
    moveObject: function(curRes, curPar, tarPar, tarRelIdx) {
        ...
        if (rtn) {
            if (blob)
                this.Database.updateObjectBLOB(moveObject.oid, blob[0], tar_dbtype);
            if (!modShift) {
                var newItem = this.Common.newItem();
                newItem.id = moveObject.oid;
                newItem.about = this.DataSource.getAbout(moveObject.oid, moveObject.pfid, tar_dbtype);
        ...
        if (rtn && modShift) {
            ...
            rtn = this.DataSource.moveItem(curRes, curPar, tarPar, tarRelIdx);
            if (rtn)
                rtn = this.Database.removeObject({oid: cur_oid, pfid: cur_pfid}, cur_dbtype);
            if (rtn) {
    ...
    capture: function(aXferString, aRow, aOrient) {
        ...
        if (!url_title || url == url_title)
            url_title = "";
        var basedir = window.top.bitsTreeListService.getBaseDir();
        var res_uri = this.Common.convertFilePathToURL(basedir.path);
        if (url.indexOf(res_uri) >= 0)
            return;
        if (this.originalTarget && this.originalTarget.nodeName == "IMG" && this.originalTarget.hasAttribute('oid') && this.originalTarget.hasAttribute('dbtype'))
            return;
        ...
        try {
            ...
            if (isSelected && aXferStringArr.length != 1)
                isSelected = false;
            if (isSelected && win.document.contentType == "application/pdf")
                isSelected = false;
        } catch (ex) {
        }
        ...
        if (this.modAlt && isEntire) {
            ...
            if (url.indexOf("http://") == 0 ||
                ...
                if (splitFileName[1].toLowerCase() == "xml" || splitFileName[1].toLowerCase() == "zip")
                    importFlag = mcXmlImportService.loadFile(res[0], res[1], aRow, url);
                if (!importFlag && !mcXmlImportService.isExportFile) {
                        ...
                        if (this.originalTarget.ownerDocument != this.gBrowser.contentDocument) {
                            ...
                            if (win.frames != null) {
                                ...
                                for (i = 0; i < win.frames.length; i++) {
                                    if (this.originalTarget.ownerDocument == win.frames[i].document)
                                        break;
                                }
                    ...
                    if (tmpOriginalTarget)
                        this.originalTarget = tmpOriginalTarget;
                ...
                if (isSelected) {
                    ...
                    setTimeout(function() {
                        ...
                        if (!rtn)
                            window.top.bitsMarkingCollection.Common.alert(mcMainService.STRING.getString("ERROR_INVALID_URL") + "\n" + url);
                    }, 0);
                }
            }
        }
    ...
    loadHTML: function(aEvent) {
        ...
        if (title)
            mcTreeDNDHandler.DataSource.setProperty(mcTreeDNDHandler._oRes, "title", title);
        if (icon)
            mcTreeDNDHandler.DataSource.setProperty(mcTreeDNDHandler._oRes, "icon", icon);
        if (title || icon) {
            mcTreeDNDHandler.DataSource.flush();
            mcTreeHandler.TREE.builder.rebuild();
            ...
            if (obj && obj.length > 0)
                oid_property = obj[0].oid_property;
            if (icon)
                oid_property += "<ICON>" + icon + "</ICON>";
    ...
    changeNodeStyle: function(pRes) {
        ...
        if (!source)
            source = bitsMarker.id_key + dbtype + id;
    ...
    init: function() {
        try {
            ...
            for (i = 0; i < listRes.length; i++) {
                if (listRes[i].Value == this.DataSource.ABOUT_ROOT)
                    continue;
...
var mcTreeMultiDocumentService = {
    get BUTTON() {
        return document.getElementById("mcToolbarMultiDocumentButton");
    },
    get DataSource() {
        return window.top.bitsObjectMng.DataSource;
    },
...
var mcTreeViewModeService = {
    ...
    dbupdateTimer: null,
    get BUTTON() {
        return document.getElementById("mcToolbarViewModeButton");
    },
    get SINGLE() {
        return document.getElementById("mcToolbarViewModeItemSingle");
    },
    get ALL() {
        return document.getElementById("mcToolbarViewModeItemAll");
    },
    get EACH() {
        return document.getElementById("mcToolbarViewModeItemEach");
    },
    get TITLE() {
        return window.top.document.getElementById("sidebar-title");
    },
    get PROGRESS() {
        return document.getElementById("mcToolbarProgressmeter");
    },
    get DataSource() {
        return window.top.bitsObjectMng.DataSource;
    },
    ...
    init: function() {
        ...
        if (this.dbupdateTimer)
            clearTimeout(this.dbupdateTimer);
    },
    done: function() {
    ...
    select: function(aEvent) {
        try {
            ...
            if (url.indexOf("chrome:") < 0 && url.indexOf("about:") < 0 && mcTreeViewModeService._openlist[url] == undefined) {
                if (mcTreeViewModeService.viewmode == "single")
                    mcTreeViewModeService.rebuild();
            }
    ...
    dbupdate: function() {
        if (mcTreeViewModeService.dbupdateTimer)
            clearTimeout(mcTreeViewModeService.dbupdateTimer);
        var rtnUpdate = false;
        if (!rtnUpdate)
            rtnUpdate = mcTreeViewModeService.Database._updateCheck(mcTreeViewModeService.Database._defaultMode);
        if (rtnUpdate)
            mcTreeViewModeService.rebuild();
    ...
    selectTreeObject: function(aOID, aDBTYPE, aPFID) {
        ...
        if (mcItemView.isChecked) {
            ...
            try {
                ...
                if (searchRes) {
                    ...
                    for (var i = resArr.length - 1; i >= 0; i--) {
                        ...
                        if (selectIdx >= 0 && !mcTreeHandler.TREE.view.isContainerOpen(selectIdx))
                            mcTreeHandler.TREE.view.toggleOpenState(selectIdx);
                    }
                    ...
                    if (selectIdx >= 0) {
                        ...
                        setTimeout(function() {
                            mcItemView.setSelection(click_oid, click_dbtype);
                        }, 0);
                    }
                }
            } catch (e) {
            ...
            if (!searchRes && searchRes1)
                searchRes = searchRes1;
            if (searchRes) {
                var resArr = [];
                var parentRes = searchRes;
                do {
    ...
    changeTitle: function() {
        ...
        if (rootfolder && rootfolder != "")
            foldres = this.Database.getFolderFormID(rootfolder);
        if (foldres)
            title = foldres[0].fid_title;
        if (title == "")
            title = mcMainService.STRING.getString("DEFAULT_FOLDER");
        var button_title = mcMainService.STRING.getString("APP_DISP_TITLE");

        // DASISH modification
        // this.TITLE.setAttribute("value",button_title + " [ " + title + " ]");
        this.TITLE.setAttribute("value", "My Annotations");
        this.TITLE.setAttribute("style", "font-size: 1.2em; text-align: center; margin: 2pt 28pt 2pt 0;");
    ...
    onClick: function(aEvent) {
        ...
        if (mcTreeViewModeService.dbupdateTimer)
            clearTimeout(mcTreeViewModeService.dbupdateTimer);
        if (viewmode == "each")
            mcTreeHandler.TREE.setAttribute("ref", this.DataSource.ABOUT_ROOT); //暫定処理
        this.rebuild();
        if (viewmode == "each")
            mcTreeHandler.openAllFolders();
        aEvent.stopPropagation();
    ...
    rebuild: function() {
        try {
            ...
            if (currentTreeIndex >= 0)
                currentTreeRes = mcTreeHandler.TREE.builderView.getResourceAtIndex(currentTreeIndex);
            ...
            while (resEnum.hasMoreElements()) {
                ...
                if (aRes.Value != this.DataSource.ABOUT_ROOT)
                    this.DataSource.removeResource(aRes);
            }
            ...
            if (mcTreeViewModeService.viewmode == "each") {
                ...
                if (currentTreeIndex < 0)
                    currentTreeIndex = 0;
                try {
                    var selectRes = mcTreeHandler.TREE.builderView.getResourceAtIndex(currentTreeIndex);
                } catch (ex2) {
                    selectRes = null;
                }
                if (!selectRes)
                ...
                if (firstVisibleRow >= 0)
                    mcTreeHandler.TREE.treeBoxObject.scrollToRow(firstVisibleRow);
                mcPropertyView.dispProperty(mcTreeHandler.object);
                this.showShortcutKey();
                setTimeout(function() {
                    mcTreeRdfRebuildItem.init();
                }, 0);
                return;
            }
        } catch (ex) {
    ...
    showShortcutKey: function() {
        ...
        if (show_shortcut) {
            ...
            for (rcnt = 0; rcnt < resAllFolder.length; rcnt++) {
                ...
                if (fRes.Value == this.DataSource.ABOUT_ROOT)
                    continue;
                ...
                if (!fid || !dbtype || type == "localfolder")
                    continue;
...
var mcTreeImageTooltip = {
    get tooltipRow() {
        return window.top.bitsImageTooltip.tooltipRow;
    },
    init: function() {
...
var mcTreeRdfRebuildItem = {
    ...
    init: function() {
        try {
            ...
            if (rdf && rdf != "") {
                ...
                parser = undefined;
                if (this.rdfdoc)
                    this.nsResolver = this.rdfdoc.createNSResolver(this.rdfdoc.ownerDocument == null ? this.rdfdoc.documentElement : this.rdfdoc.ownerDocument.documentElement);
                rdf = undefined;
            }
            ...
            if (this.progressWindow && !this.progressWindow.closed) {
                if (this.resOpenFolder.length > 0 || this.resAllFolder.length > 0) {
                    if (this.progressWindow.setStatus)
                        this.progressWindow.setStatus(mcMainService.STRING.getString("MSG_LOADING_SCRAP") + "... [ " + (this.resOpenFolder.length + this.resAllFolder.length) + " ]");
                } else {
                    if (this.progressWindow && !this.progressWindow.closed)
                        this.progressWindow.close();
                    this.progressWindow = null;
        ...
        try {
            ...
            if (this.progressWindow && !this.progressWindow.closed) {
                if (fRes) {
                    ...
                    if (this.progressWindow.setStatus)
                        this.progressWindow.setStatus(mcMainService.STRING.getString("MSG_LOADING_SCRAP") + "... [ " + (this.resOpenFolder.length + this.resAllFolder.length + 1) + " ]");
                }
            }
            if (!fRes) {
                this.done();
            ...
            if (this.resLoadFolder[fRes.Value] == undefined) {
                ...
                if (this.itemObjects && this.itemObjects.length > 0 && this.folderFilterHash[fid]) {
                    ...
                    for (j = 0; j < this.itemObjects.length; j++) {
                        ...
                        if (this.folderFilterHash[obj.pfid] && !obj.oid_title.match(this.folderFilterHash[obj.pfid]))
                            continue;
                        newObj.push(obj);
                    }
                ...
                if (this.itemObjects && this.itemObjects.length > 0) {
                    ...
                    if (this.rdfdoc && this.nsResolver) {
                        ...
                        if (xPathXML && xPathXML.singleNodeValue)
                            this.xmlNode = xPathXML.singleNodeValue;
                    }
                    if (this.xmlNode) {
                        this.timerid = setTimeout(function() {
                            mcTreeRdfRebuildItem.execItem(fRes);
                        }, 0);
                        return;
                    }
                } else {
    ...
    setTreeEvent: function() {
        mcTreeHandler.TREE.onmousedown = function(e) {
            mcTreeHandler.onMousedown(e);
        }
        mcTreeHandler.TREE.onmouseup = function(e) {
            mcTreeHandler.onMouseup(e);
        }
        mcTreeHandler.TREE.onmousemove = function(e) {
            mcTreeHandler.onMousemove(e);
        }
        mcTreeHandler.TREE.onmouseout = function(e) {
            mcTreeHandler.onMouseout(e);
        }
        mcTreeHandler.TREE.ondblclick = function(e) {
            mcTreeHandler.onDblClick(e);
        }
        mcTreeHandler.TREE.onkeydown = function(e) {
            mcTreeHandler.onKeyDown(e);
        }
        mcTreeHandler.TREE.onkeyup = function(e) {
            mcTreeHandler.onKeyUp(e);
        }
        mcTreeHandler.TREE.onkeypress = function(e) {
            mcTreeHandler.onKeyPress(e);
        }
        mcTreeHandler.TREE.onclick = function(e) {
            mcTreeRdfRebuildItem.onclick(e);
        }
        var info = Components.classes["@mozilla.org/xre/app-info;1"].getService(Components.interfaces.nsIXULAppInfo);
        var app_version = parseFloat(info.version);
        if (app_version < 3.1) {
...
var mcItemView = {
    ...
    get _bitsItemViewToolbar() {
        return document.getElementById("bitsItemViewToolbar");
    },
    get modAlt() {
        return this._bitsItemView.modAlt;
    },
    get modShift() {
        return this._bitsItemView.modShift;
    },
    get modCtrl() {
        return this._bitsItemView.modCtrl;
    },
    get modMeta() {
        return this._bitsItemView.modMeta;
    },
    get isChecked() {
        return this._bitsItemView.isChecked;
    },
    get object() {
        return this._bitsItemView.object;
    },
    get TREE() {
        return this._bitsItemView.TREE;
    },
    get POPUP() {
        return document.getElementById("mcPopupFolder");
    },
    get CONTEXTMENU() {
        return document.getElementById("mcPopupFolderItemViewMenu");
    },
    get FAVICON_POPUP() {
        return document.getElementById("mcPopupFolderItemViewFaviconMenupopup");
    },
    get ITEM_POPUP() {
        return document.getElementById("mcPopupFolderItemViewItemMenupopup");
    },
    get idMENUITEM() {
        return "mcPopupFolderItem_";
    },
    init: function(aEvent) {
        this._bitsItemView.init(aEvent);
        if (this._bitsItemView.isChecked) {
            this.CONTEXTMENU.removeAttribute("hidden");
            if (this._bitsItemViewToolbar && mcTreeViewModeService.BUTTON)
                this._bitsItemViewToolbar.appendChild(mcTreeViewModeService.BUTTON);
    ...
    initMenu: function(aEvent) {
        var contextmenu_mode = nsPreferences.copyUnicharPref("wiredmarker.contextmenu.mode");
        if (contextmenu_mode == "legacy") {
            if (this._bitsItemView.VTTB)
                this._bitsItemView.VTTB.removeAttribute("hidden");
        } else {
            if (this._bitsItemView.VTTB)
                this._bitsItemView.VTTB.setAttribute("hidden", "true");
            ...
            if (this._bitsItemView.TREE && this.ITEM_POPUP && !this.ITEM_POPUP.hasChildNodes()) {
                ...
                for (var i = 0; i < treecols.length; i++) {
                    if (treecols[i].hasAttribute("ignoreincolumnpicker") && treecols[i].getAttribute("ignoreincolumnpicker") == "true")
                        continue;
                    var menuitem = document.createElement("menuitem");
    ...
    _popupMenu: function(aEvent) {
        if (this._bitsItemView.VTTB && this.FAVICON_POPUP && this.FAVICON_POPUP.hasChildNodes()) {
            var menuitems = this._bitsItemView.VTTB.getElementsByTagName("menuitem");
            for (var i = 0; i < menuitems.length; i++) {
                if (!menuitems[i].hasAttribute("checked") || menuitems[i].getAttribute("checked") == "false")
                    continue;
                try {
    ...
    _commandMenuFavicon: function(aEvent) {
        if (!aEvent.target.id.match(new RegExp("^" + this.idMENUITEM + "(.+$)")))
            return;
    ...
    mergeObject: function(srcRes, dstRes) {
        ...
        var dst_title;
        if (srcRes)
            src_title = mcTreeHandler.DataSource.getProperty(srcRes, "title");
        if (dstRes)
            dst_title = mcTreeHandler.DataSource.getProperty(dstRes, "title");
            ...
            if (aTopic == 'nsPref:changed') {
                switch (aPrefstring) {
                    case "wiredmarker.last_update":
                        ...
                        var arr = value.split("\t");
                        if (inittime != arr[2]) {
                            if (mcDatabaseObserver.timerid)
                                clearTimeout(mcDatabaseObserver.timerid);
                            mcDatabaseObserver.timerid = setTimeout(function() {
                                mcDatabaseObserver.timerid = null;
